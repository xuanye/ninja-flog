...
export class TileUtilities {
  loadTiledMap(jsonTiledMap: { data: any }) {
    ...
    // 获取
    tiledMap.tilesets.forEach((tileset: unknown) => {
      world.tilesets.push(tileset);
    });

    tiledMap.layers.forEach((tiledLayer: any) => {
      // Make a group for this layer and copy
      // all of the layer properties onto it.
      const layerGroup: Record<string, any> = { data: [] };
        ...
        tiledLayer.data.forEach((gid: number, index: number) => {
          ...
          if (gid !== 0) {
            ...
            if (spacing > 0) {
              ...
              tilesetY +=
                spacing + spacing * Math.floor((gid - tileset.firstgid) / numberOfTilesetColumns);
            }
      ...
      if (tiledLayer.type === 'objectgroup') {
        tiledLayer.objects.forEach((object: any) => {
